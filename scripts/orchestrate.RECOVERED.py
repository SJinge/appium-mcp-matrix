...
def _normalize_flow_locator(locator):
    return orch_case_script.normalize_flow_locator(locator, _text_field)


def _normalize_flow_fallbacks(fallbacks):
    return orch_case_script.normalize_flow_fallbacks(fallbacks, _text_field)


def _flow_step_from_action(step: dict) -> dict:
    return orch_case_script.flow_step_from_action(step, _text_field)
...
def generate_case_script(app_id: str, platform: str, case: dict, version: str = None):
    ...
    preconds = [step.get("text", "") for step in case.get("steps", []) if step.get("type") == "PRECOND"]
    asserts = [
        {
            "text": step.get("text", ""),
        ...
        for step in _passed_assert_steps(case)
    ]
    content = f'''# Auto-generated by orchestrate. Edit with care.
META = {repr(meta)}
PRECONDITIONS = {repr(preconds)}
ASSERTIONS = {repr(asserts)}


def execute(ctx):
    results = []
    for item in ASSERTIONS:
        ok = ctx["assert_evidence_present"](item["verify_method"], item["evidence"])
        results.append({{
            "type": "ASSERT",
            "text": item["text"],
            "passed": ok,
        ...
        if not ok:
            return {{
                ...
                "module": META["module"],
                "passed": False,
                "note": f'script assertion failed: {{item["evidence"]}}',
                "steps": results,
        ...
        if step_type.startswith("assert_"):
            ...
            if not ok:
                return {
                    ...
                    "passed": False,
                    "note": f"script assertion failed: {item.get('value', '')}",
                    "steps": results,
                    "script_source": "script",
                }
            continue
...
def execute_case_with_fallback(app_id: str, platform: str, udid: str, entry: dict,
    ...
    is_android = normalized_platform == "android"
    use_case_script = (
    ...
    if script_path:
        try:
            ...
            script_error = script_result.get("note", "script failed")
        except Exception:
...
def build_prompt(app_id: str, udid: str, platform: str, entries: list,
    ...
    return (
        ...
        f"6. ACTION/PRECOND 步也要写结构化轨迹字段，供后续生成可执行脚本："
        f"script_action（如 tap/input/back/route/prepare_state）、locator（如 by=id/text,value=...）、"
        f"locator_fallbacks（可选候选定位数组）、input_value（有输入时必填）、page_hint/route_target（有跳页时尽量填写）。\n\n"
    ...
    if "ios" in platforms:
        for d in devices["ios"]:
            ...
            if ios_wda.ensure_wda_ready(
                ...
                bundle_id = PKG_NAMES.get(app_id, "")
                if grant_ios_network_permission(d["udid"], bundle_id):
                    ios_ok.append(d)
                    wda_ports[d["udid"]] = port
                else:
                    _notify(app_id, version,
                            f"⚠️ iOS {d['udid']} 网络权限授予失败，环境阻断，跳过（不写结果表）")
                    log.warning(f"iOS {d['udid']} network permission grant failed, skipping (env blocked)")
            else:
                _notify(app_id, version,
