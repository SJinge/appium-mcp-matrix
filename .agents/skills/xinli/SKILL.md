...
name: xinli
description: |
  高途心理 app 移动端自动化测试全生命周期 skill，支持 Android 和 iOS 双端。
  触发词："心理用例"、"执行测试"、"测试心理"、"xinli test"、"用例 ID"
---

# 角色规范

本 skill 基于 **appium-expert** 角色规范运行。以下仅包含高途心理 app 专用执行流程。
...
| packageName | `com.gaotu100.xinli` | — |
| bundleId | — | 首次使用请确认 |
...
- **路径 D（Monkey 测试）**：询问以下参数后，按 `common/monkey.md` 执行：
  ...
  - `APP_ID=xinli`
  - `PKG=com.gaotu100.xinli`（Android）或 `PKG=<bundleId>`（iOS，首次使用请确认）
  - `ACCOUNT=12100000000`
  - `VERSION`：从以下命令提取：
    ```bash
    adb shell dumpsys package com.gaotu100.xinli | grep versionName | head -1
    ```
...
> 见 [../../common/permission.md](../../common/permission.md)，`packageName = com.gaotu100.xinli`

---

## 第二步：创建 Appium Session

> capabilities 模板见 [../../common/device.md](../../common/device.md)，填入高途心理专用参数
...
**Android：**
```bash
adb -s <device> shell am force-stop com.gaotu100.xinli
adb -s <device> shell am start -n com.gaotu100.xinli/.ui.activity.SplashActivity
```
...
- **需要登录** → 见 [../../common/login.md](../../common/login.md)
  > 调用参数：`app_id=xinli`，`platform=<平台>`，`method=password`，`phone=<账号>`，`password=<密码>`
...
SHOT_DIR="$HOME/mcp_shots/xinli/$(date +%Y%m%d_%H%M)_${CASE_ID:-manual}"
mkdir -p "$SHOT_DIR"
```
...
```bash
# 获取 App 版本（Android）
APP_VERSION=$(adb -s <UDID> shell dumpsys package com.gaotu100.xinli \
