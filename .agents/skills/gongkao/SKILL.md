...
name: gongkao
description: |
  高途公职 app 移动端自动化测试全生命周期 skill，支持 Android 和 iOS 双端。
  触发词："公职用例"、"执行测试"、"测试公职"、"gongkao test"、"用例 ID"
---

# 角色规范

本 skill 基于 **appium-expert** 角色规范运行。以下仅包含高途公职 app 专用执行流程。
...
| packageName | `com.gaotu100.gongkao` | — |
| bundleId | — | 首次使用请确认 |
...
- **路径 D（Monkey 测试）**：询问以下参数后，按 `common/monkey.md` 执行：
  ...
  - `APP_ID=gongkao`
  - `PKG=com.gaotu100.gongkao`（Android）或 `PKG=<bundleId>`（iOS，首次使用请确认）
  - `ACCOUNT=12100000000`
  - `VERSION`：从以下命令提取：
    ```bash
    adb shell dumpsys package com.gaotu100.gongkao | grep versionName | head -1
    ```
...
> 见 [../../common/permission.md](../../common/permission.md)，`packageName = com.gaotu100.gongkao`

---

## 第二步：创建 Appium Session

> capabilities 模板见 [../../common/device.md](../../common/device.md)，填入高途公职专用参数
...
**Android：**
```bash
adb -s <device> shell am force-stop com.gaotu100.gongkao
adb -s <device> shell am start -n com.gaotu100.gongkao/.ui.activity.SplashActivity
```
...
- **需要登录** → 见 [../../common/login.md](../../common/login.md)
  > 调用参数：`app_id=gongkao`，`platform=<平台>`，`method=password`，`phone=<账号>`，`password=<密码>`
...
SHOT_DIR="$HOME/mcp_shots/gongkao/$(date +%Y%m%d_%H%M)_${CASE_ID:-manual}"
mkdir -p "$SHOT_DIR"
```
...
在上下文中维护以下变量（非 shell 变量，由 Codex 在对话上下文中跟踪）：
...
```bash
# 获取 App 版本（Android）
APP_VERSION=$(adb -s <UDID> shell dumpsys package com.gaotu100.gongkao \
...
- **截图必须加 `maxWidth: 800`**：Android/iOS 高分辨率设备截图超 2000px 会触发 Codex 多图限制报错，所有 `appium_screenshot` 调用均须传入 `maxWidth: 800`
