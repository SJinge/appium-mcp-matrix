...
name: jingpin
description: |
  高途高中 app 移动端自动化测试全生命周期 skill，支持 Android 和 iOS 双端。
  触发词："高中用例"、"执行测试"、"测试高中"、"jingpin test"、"用例 ID"
---

# 角色规范

本 skill 基于 **appium-expert** 角色规范运行。以下仅包含高途高中 app 专用执行流程。
...
| packageName | `com.gaotu100.jingpin` | — |
| bundleId | — | 首次使用请确认 |
...
> 见 [../../common/permission.md](../../common/permission.md)，`packageName = com.gaotu100.jingpin`

---

## 第二步：创建 Appium Session

> capabilities 模板见 [../../common/device.md](../../common/device.md)，填入高途高中专用参数
...
**Android：**
```bash
adb -s <device> shell am force-stop com.gaotu100.jingpin
adb -s <device> shell am start -n com.gaotu100.jingpin/.ui.activity.SplashActivity
```
...
- **需要登录** → 见 [../../common/login.md](../../common/login.md)
  > 调用参数：`app_id=jingpin`，`platform=<平台>`，`method=password`，`phone=<账号>`，`password=<密码>`
...
SHOT_DIR="$HOME/mcp_shots/jingpin/$(date +%Y%m%d_%H%M)_${CASE_ID:-manual}"
mkdir -p "$SHOT_DIR"
```
...
```bash
# 获取 App 版本（Android）
APP_VERSION=$(adb -s <UDID> shell dumpsys package com.gaotu100.jingpin \
